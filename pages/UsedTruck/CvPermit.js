const cvPermitData = require('../../testData/UsedTruck/CvPermitData.json');
...
/**
 * CV Permit (/en/vehicle-permit-check).
 *
 * Scope for now, per request: only the lead forms on this page — not the
 * permit-info content itself, which is out of scope until asked for.
 *
 * Live click-and-inspect audit found **two** distinct lead forms:
 * 1. "Check Offers" — the shared CheckOffersLead modal (`input#name[name=
 *    "name"]`), same as elsewhere on the site.
 * 2. "Check Permit Info" — after filling a vehicle registration number (the
 *    page's primary tool input), this opens a **different** shared
 *    component, GetInformationLead (`helpers/getInformationLeadFiller.js`):
 *    name/phone matched by `name` attribute only (no id), phone is
 *    `type="number"`, plus a required truck-brand autocomplete ("Brands*").
 *    Confirmed live via network intercept that submitting it fires a real
 *    POST to a `.../vehicle-permit...` endpoint (the same backend component
 *    reused on Vehicle Report and E Challan) and returns 2xx.
...
class CvPermit {
  constructor(lang = 'en') {
    this.lang = lang;
    this.page = cvPermitData.CvPermit[lang] || cvPermitData.CvPermit.en;
  ...
  static get supportedLanguages() {
    return Object.keys(cvPermitData.CvPermit);
  }
  ...
  // --- Check Permit Info (GetInformationLead) ---
  ...
  /**
   * Fill the vehicle registration number precondition, then open the Get
   * Information lead form. Typing a complete, valid-format registration
   * number sometimes auto-opens this form immediately (no click needed) —
   ...
   * appearing IS the real, honest, sufficient success signal — the earlier
