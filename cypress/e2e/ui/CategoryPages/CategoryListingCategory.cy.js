...
// The nav's "Category" submenu, plus 3 Wheeler (its own top-level nav link,
// grouped here for run-size purposes since it shares the identical page
// pattern). Split into its own file — rather than one 28-page mega-spec —
// because running all 28 category-pattern pages back-to-back in a single
// Cypress browser session caused cumulative slowdown severe enough that
// dropdown interactions on whichever pages ran last (confirmed by moving
// the same page later in a combined run and watching it start failing)
// exceeded even a 30s retry window. Each group here is small enough to run
// in one session without that degradation.
const PAGE_KEYS = [
  'tippers',
  'trailers',
  'miniTrucks',
...
/**
 * Category listing pages — Tippers, Trailers, Mini Trucks, Pickups, Transit
 * Mixer, Auto Rickshaw, Tempo Traveller, 3 Wheeler (see
 ...
 *
 * Run:
 * - npm run test:categoryListing:category
 */
