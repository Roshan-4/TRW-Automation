...
// The nav's "Fuel Type" submenu (Diesel/Petrol/CNG/LPG/LNG/Hydrogen/Bi Fuel
// — Electric is already covered separately by ElectricVehicle.js). Split
// into its own file — rather than one 28-page mega-spec — because running
// all 28 category-pattern pages back-to-back in a single Cypress browser
// session caused cumulative slowdown severe enough that dropdown
// interactions on whichever pages ran last (confirmed by moving the same
// page later in a combined run and watching it start failing) exceeded
// even a 30s retry window. Each group here is small enough to run in one
// session without that degradation.
const PAGE_KEYS = ['diesel', 'petrol', 'cng', 'lpg', 'lng', 'hydrogen', 'biFuel'];

/**
 * Category listing pages — Diesel, Petrol, CNG, LPG, LNG, Hydrogen, Bi Fuel
 * Trucks (see pages/CategoryPages/CategoryListing.js for the full
 * live-audited CTA findings, shared across all four CategoryListing* spec
 * files). Scope for now: only the lead form(s) on each page. Every page in
 * this group has both Check Truck Price and Call Now.
 *
 * Run:
 * - npm run test:categoryListing:fuelType
 */
